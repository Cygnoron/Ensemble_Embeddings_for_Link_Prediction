...
for rank in "${params_rank[@]}"; do
  for subgraph_size_range in "${params_subgraph_size_range[@]}"; do
    for subgraph_amount in "${params_subgraph_amount[@]}"; do
      ...
      sbatch train_NELL-995-h100_TransE.sh --rank "$rank" --aggregation_method "$aggregation_method" --subgraph_amount "$subgraph_amount" --subgraph_size_range "$subgraph_size_range"
      sbatch train_NELL-995-h100_DistMult.sh --rank "$rank" --aggregation_method "$aggregation_method" --subgraph_amount "$subgraph_amount" --subgraph_size_range "$subgraph_size_range"
