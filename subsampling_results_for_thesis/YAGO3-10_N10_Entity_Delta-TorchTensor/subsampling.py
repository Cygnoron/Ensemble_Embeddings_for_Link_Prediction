            ...
            with (open(os.path.abspath(f"{dataset_out_dir}\\sub_{subgraph_num:03d}\\train.pickle"),
                ...
                with open(config_directory, 'a') as config_file:
                    ...
                    logging.critical(f"Length of delta: {len(delta_triples)}\t"
                                     f"Total removed duplicates from delta: {removed_ids_counter}")
...
def calculate_delta(subgraph_size_range, dataset, subgraph_num, subgraph_amount, entity_set, relation_name_set,
    ...
    if sampling_method[0] == Constants.ENTITY_SAMPLING[0]:
        ...
        progress_bar = None
        # -- sampling process --
        while delta.size(0) / len(dataset) <= subgraph_size:
            ...
            if (len(relation_name_ids_unused) > 0) and (delta.size(0) / len(dataset) >= (subgraph_size * 0.9)):
        ...
        progress_bar.close()
        logging.info(f"Current subgraph size {delta.size(0) / len(dataset)} reached or exceeded target size "
                     f"{subgraph_size}.")
    ...
    # -- delta creation for Feature Sampling --
        ...
        while delta.size(0) / len(dataset) < subgraph_size:
            ...
            if (len(relation_name_set) == 0) or (len(entity_set) == 0):
                ...
                if delta.size(0) / len(dataset) >= subgraph_size:
                    progress_bar.close()
                    logging.info(
                        f"Current subgraph size {delta.size(0) / len(dataset)} reached or exceeded target size "
                        f"{subgraph_size}.")
                    logging.info(f"Created subgraph {subgraph_num} with {len(entity_ids_unused)} entities and "
                                 f"{len(relation_name_ids_unused)} relation names left unused and a relative subgraph "
                                 f"size {delta.size(0) / len(dataset)}.")
                    return delta, entity_ids_unused, relation_name_ids_unused, subgraph_relation_names
                else:
                    ...
                    logging.info(f"No relation names left to sample (subgraph size "
                                 f"{round(delta.size(0) / len(dataset), 3)}/{subgraph_size}). Adding additional relation "
...
def sampling(entity_set, relation_name_set, entity_ids_unused, relation_name_ids_unused, dataset, delta,
    ...
    try:
        ...
        if (enforce_relation_names and sampling_method == Constants.ENTITY_SAMPLING and
            ...
            for enforced_relation_name_id in relation_name_ids_iterator_set:
                ...
                delta = torch.cat((delta, torch.tensor([sample]).int()), dim=0)
        ...
        delta = torch.tensor(delta.tolist() + list(sample_triple_indices), dtype=torch.int64)
    ...
    unique_before = delta.size(0)
    delta = torch.unique(delta)
    unique_after = delta.size(0)

    if unique_before != unique_after:
        removed_ids_counter += unique_before - unique_after
        # logging.critical(f"{unique_before-unique_after} triples were duplicates and were removed.")

    # update progress bar
    progress_bar.n = delta.size(0)
