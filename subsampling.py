...
def sample_graph(info_directory: str, dataset_in: str, dataset_out_dir: str, sampling_method, subgraph_amount=10,
    ...
    # directory for info and config files
    info_directory = os.path.abspath(f"{dataset_out_dir}\\results")

    util_files.delete_paths(f"{info_directory}", "Ensemble_Embedding.log")
    dataset_in_train = f"KGEmb\\data\\{dataset_in}\\train"
    ...
    try:
        ...
        for subgraph_num in range(subgraph_amount):
            ...
            with (open(os.path.abspath(f"{dataset_out_dir}\\sub_{subgraph_num:03d}\\train.pickle"),
                ...
                source_dir = f"KGEmb\\data\\{dataset_in}"
